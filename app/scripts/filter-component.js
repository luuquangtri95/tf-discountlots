const { debounceTime, addEvent } = theme;
const customElementsName = "filter-component";
const TIMEOUT = 300;
	...
	window.customElements.define(
		...
		class extends HTMLElement {
			...
			addEvents() {
				this.addEventListener("click", (e) => {
					if (e.target.closest(".js-auto-submit")) {
						...
						if (e.target.closest("[name=sort_by], [name=sort_by_clone]")) {
							...
							document.querySelectorAll(".sort-value").forEach((item) => {
								item.innerHTML = label.querySelector(".text")
					...
					if (e.target.closest(".view-item")) {
						...
						this.querySelectorAll(".view-item").forEach((e) =>
					...
					if (e.target.closest(".jsFilter")) {
						...
						main_filter?.classList.contains("active")
							? main_filter.classList.remove("active")
							: main_filter.classList.add("active");
						e.target.closest(".jsFilter").classList.contains("active")
							? e.target.closest(".jsFilter").classList.remove("active")
							: e.target.closest(".jsFilter").classList.add("active");
					}
					...
					if (e.target.closest(".jsFilterRemove")) {
						...
						if (this.isRunning != null) clearTimeout(this.isRunning);
			...
			checkBanner() {
				//remve all
				document
					.querySelectorAll(".collection__main-products .info-card")
					.forEach((ele) => ele.remove());

				//insert
				let page_banners = document
					.querySelector("#collection-banner")
					?.content.querySelectorAll(".info-card");
				//console.log('page_banners', page_banners)
				page_banners?.forEach((e) => {
					let index = e.dataset.position; // Desired position
					...
					const reference = parent.children[index] || null; // If index is out of bounds, append at the end
					...
					let splitCharacter =
						this.filterType == "attribute"
							...
							: !searchParams.includes("?") ||
							  (searchParams.includes("?") && searchParams.split("?")[0].length)
							? "/"
							: "";
					history.pushState(
						{ searchParams },
