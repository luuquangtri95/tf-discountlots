(() => {
  // app/scripts/component/productForm.js
  !window.customElements.get("product-form") && customElements.define(
    "product-form",
    class extends HTMLElement {
      ...
      additionalPriceObserver() {
        ...
        this.spoEmbed.addEventListener("change", (e) => {
          setTimeout(() => {
            ...
            if (!mainPrice)
              return;
            if (this.originalPrice == void 0)
              this.originalPrice = +mainPrice.dataset.value;
      ...
      async updateContentNew() {
        try {
          ...
          Array.from(this.querySelectorAll(".jsVariantUpdate")).forEach((node) => {
            const selector = Array.from(node.classList).map((cls) => `.${CSS.escape(cls)}`).join("");
      ...
      linkOptions() {
        ...
        if (!Array.from(this.querySelectorAll("select[name*=option], input[name*=option]:checked")).filter((i2) => i2 != null).length)
          return;
