(() => {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!detailsOpening.length || e.target.closest("details"))
      return;
    detailsOpening.forEach((item) => {
      if (item.closest("lazy-script, accordion-component, .section-announcement-bar"))
        return;
      item.open = false;
    });
  });
  !window.customElements.get("accordion-component") && window.customElements.define("accordion-component", class extends HTMLElement {
    constructor() {
      super();
